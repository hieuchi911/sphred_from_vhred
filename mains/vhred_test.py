...
class VHREDTester(object):
    ...
    def realtime_test(self, model, dataLoader, sess): # initials is the first 2 utterances (ids), user will input the third, model generate 4th response
        ...
        f = input("Choose flag G (generic) or D (detailed): ")
        flag = [0.0, 1.0]
        if f == "D":
            flag = [1.0, 0.0]
        utt = []
        ...
        enc_inp_realtime_test, labels = dataLoader.set_test_data(utterances, flag)
        infer_decoder_ids_general = model.infer_decoder_session(sess, enc_inp_realtime_test, labels, [flag])
        resp = ids_to_words(infer_decoder_ids_general[0][0], dataLoader.id_to_word, is_pre_utterance=False)
        print("Processing...")
        print("\t\tSPHRED Bot:\t", resp)
        utt.append(resp)
        while True:
            user_res = input("\t\tYou:\t")
            if user_res == "<<exit>>":
